...
// Function to fetch Discord server stats
async function fetchDiscordStats() {
    try {
        // Use Discord invite API to get server stats
        const inviteCode = 'TQvTA2ANXm';
        const response = await axios.get(`https://discord.com/api/v10/invites/${inviteCode}?with_counts=true`);
        
        if (response.data) {
            discordStats = {
                members: response.data.approximate_member_count || 0,
                online: response.data.approximate_presence_count || 0,
                lastUpdated: Date.now()
            };
            console.log('Discord stats updated:', discordStats);
        }
    } catch (error) {
...
// Fetch stats immediately on startup
fetchDiscordStats();
...
router.post('/link', isAuthed, async (req, res) => {
    ...
    if (!process.env.DISCORD_CLIENT_ID || process.env.DISCORD_CLIENT_ID === 'placeholder') {
        return res.status(503).json({ error: 'Discord OAuth is not configured on the server. Please contact an administrator.' });
    }
