...
const { roundDecimal, emitBalance } = require('../../../utils');
...
async function startCoinflip(coinflip) {
    ...
    try {
        ...
        const loser = coinflip[winnerSide === 'fire' ? 'ice' : 'fire'];
        ...
        await doTransaction(async (connection, commit) => {
            ...
            // Update winner balance and fetch new balance
            let newWinnerBalance = 0;
            if (winner.role != 'BOT') {
                await connection.query("UPDATE users SET balance = balance + ? WHERE id = ?", [winnings, winner.id]);
                const [[winnerData]] = await connection.query("SELECT balance FROM users WHERE id = ?", [winner.id]);
                newWinnerBalance = winnerData.balance;
            }
            ...
            if (winner.role != 'BOT') {
                emitBalance(winner.id, 'set', newWinnerBalance);
            }
