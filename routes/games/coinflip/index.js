    ...
    try {
        ...
        await doTransaction(async (connection, commit) => {
            ...
            if (bot) {
                ...
                const [botUsers] = await connection.query('SELECT id, username, xp, role, anon FROM users WHERE role = ? LIMIT 1', ['BOT']);
                if (!botUsers || botUsers.length === 0) {
                    console.error('No BOT users found in database');
                    return res.status(500).json({ error: 'NO_BOTS_AVAILABLE' });
                }
                user = botUsers[0];
            } else {
                ...
                emitBalance(user.id, 'set', roundDecimal(user.balance - coinflip.amount));
                await xpChanged(user.id, user.xp, roundDecimal(user.xp + xp), connection);
            }
