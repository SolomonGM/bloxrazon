...
const { roundDecimal, getUserLevel, sendLog, xpChanged, emitBalance } = require('../../../utils');
...
router.post('/create', isAuthed, apiLimiter, async (req, res) => {
    ...
    try {
        ...
        await doTransaction(async (connection, commit) => {
            ...
            emitBalance(user.id, 'set', roundDecimal(user.balance - cost));
            ...
            await doTransaction(async (connection, commit) => {
                ...
                if (bot) {
                    ...
                    if (battle.entryPrice > 0) {
                        ...
                        emitBalance(user.id, 'set', roundDecimal(user.balance - battle.entryPrice));
                        await xpChanged(user.id, user.xp, roundDecimal(user.xp + xp), connection);
