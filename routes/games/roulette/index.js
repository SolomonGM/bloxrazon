...
const { roundDecimal, xpChanged, emitBalance } = require('../../../utils');
    ...
    try {
        ...
        await doTransaction(async (connection, commit) => {
            ...
            if (existing) {
                ...
                const [rouletteBetResult] = await connection.query('INSERT INTO rouletteBets (userId, roundId, color, amount) VALUES (?, ?, ?, ?)', [user.id, roulette.round.id, colorName, amount]);
        ...
        // Query the updated balance after transaction commit and emit
        const [[updatedUser]] = await sql.query('SELECT balance FROM users WHERE id = ?', [req.userId]);
        if (updatedUser) {
            console.log(`[ROULETTE] Bet placed. Emitting balance for user ${req.userId}: ${updatedUser.balance}`);
            emitBalance(req.userId, 'set', roundDecimal(updatedUser.balance));
        }
