...
router.post('/join', [isAuthed, apiLimiter], async (req, res) => {
    ...
    // if (!discordClient.bloxRazonGuild?.members.cache.has(linkedDiscord.user.id)) {
    //     discordClient.bloxRazonGuild?.members.add(linkedDiscord.discordId, { accessToken: linkedDiscord.token });
    // }
...
router.post('/tip', [isAuthed, apiLimiter], async (req, res) => {
    ...
    console.log('Rain tip endpoint hit by user:', req.userId);
    ...
    try {
        ...
        await doTransaction(async (connection, commit, rollback) => {
            ...
            console.log('Current rain:', rain);
            if (!rain || rain.ended || !rain.joinable) return res.status(404).json({ error: 'RAIN_NOT_FOUND' });
