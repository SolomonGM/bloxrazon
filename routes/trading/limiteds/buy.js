...
const { roundDecimal, formatConsoleError, getRobloxApiInstance, sendLog, newNotification, emitBalance } = require('../../../utils');
...
async function buyListing(data) {
    ...
    try {
        ...
        await doTransaction(async (connection, commit) => {
            ...
            emitBalance(seller.id, 'add', balanceToAdd);
            io.to(seller.id).emit('toast', 'success', 'You have successfully sold your item(s) on the marketplace.');
            emitBalance(buyer.id, 'set', roundDecimal(buyer.balance - listing.total));
