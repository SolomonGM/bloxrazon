...
async function cacheCryptos() {
    ...
    try {
        const [cryptos] = await sql.query("SELECT symbol, name, coingeckoId, price FROM cryptos");

        for (const crypto of cryptos) {
            if (cryptoData.currencies[crypto.symbol]) {
                cryptoData.currencies[crypto.symbol].price = crypto.price;
            }
        }
        ...
        console.error('Error caching cryptos:', e.message);
    }
