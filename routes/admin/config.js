...
async function cacheAdmin() {
    ...
    const [features] = await sql.query('SELECT id, name, enabled FROM features');
    features.forEach(e => enabledFeatures[e.name.toLowerCase()] = !!e.enabled);
...
async function checkAccountLock(user) {
    ...
    try {
        ...
        // If transactions table doesn't have deposit type yet, skip this check
        console.error('checkAccountLock error (transactions table may need updating):', error.message);
        return false;
    }
