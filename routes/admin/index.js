...
router.post('/fix-transactions-table', async (req, res) => {
    try {
        await sql.query(`
        ...
        sendLog('admin', `[\`${req.userId}\`] *${req.user.username}* fixed transactions table.`);
        res.json({ success: true, message: 'Transactions table updated successfully' });
    } catch (error) {
