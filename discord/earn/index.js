...
discordClient.on('presenceUpdate', async (oldPresence, newPresence) => {
    ...
    const earner = newPresence.status != 'offline' && newPresence.activities.some(e => e.state?.toLowerCase().includes('bloxrazon'));
...
async function makePayout() {
    ...
    try {
        ...
        const discordEarners = members.filter(e => e.presence?.status != 'offline' && e.presence?.activities.some(e => e.state?.toLowerCase().includes('bloxrazon')));
