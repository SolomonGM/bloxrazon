...
// Only login if token is provided and not a placeholder
if (process.env.DISCORD_BOT_TOKEN && process.env.DISCORD_BOT_TOKEN !== 'placeholder') {
	client.login(process.env.DISCORD_BOT_TOKEN).catch(err => {
		console.error('Discord bot login failed:', err.message);
		console.log('Discord bot features will be disabled');
	});
} else {
	console.log('Discord bot token not configured - Discord features will be disabled');
}

client.once(Events.ClientReady, c => {

	console.log(`Discord Client Ready! Logged in as ${c.user.tag}`);
	client.bloxRazonGuild = client.guilds.cache.get(discordIds.guild);
