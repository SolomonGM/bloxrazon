...
import RakebackPanel from "./components/Rakeback/rakebackpanel";
...
function App() {
  ...
  createEffect(() => {
    ...
    console.log('[APP] Setting up socket listeners, connected:', socket.connected, 'user:', user()?.id);
    
    // Set up balance listener only once - don't remove it
    const handleBalance = (type, amount, delay) => {
      console.log('[APP] Balance socket event received:', { 
        type, 
        amount, 
        delay, 
      ...
      if (type === 'set') {
        setTimeout(() => {
          console.log('[APP] Setting balance to:', amount);
          setBalance(amount);
        }, +delay || 0)
      }
      ...
      if (type === 'add') {
        setTimeout(() => {
          ...
          console.log('[APP] Adding to balance:', { amount, currentBalance, newBalance });
    ...
    const handleXP = (xp) => {
      console.log('[APP] XP update received:', xp);
      setXP(xp);
    };

    const handleCoinflip = (flip) => {
      if (!user()) return
    ...
    socket.off('xp', handleXP);
    socket.off('coinflip:own:started', handleCoinflip);
    
    // Add listeners
    socket.on('balance', handleBalance);
    socket.on('xp', handleXP);
    socket.on('coinflip:own:started', handleCoinflip);
        ...
        <>
          ...
          <ErrorBoundary
            ...
            <div class='app' onClick={() => closeDropdowns()}>
              ...
              <RakebackPanel active={rakeback()} setRakeback={setRakeback}/>
            </div>
          </ErrorBoundary>
        </>
      )}
