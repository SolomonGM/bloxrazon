...
export function WebsocketProvider(props) {
    ...
    async function connectSocket() {
        ...
        tempWs.on('disconnect', (reason) => {
            console.log('Disconnected from WS:', reason)
            if (reason !== 'io server disconnect') return

            mutate(null)
            let retrying = setInterval(() => {
                tempWs.removeAllListeners()
                tempWs = io(socketUrl, { transports: ['websocket', 'polling'], reconnection: true, reconnectionDelay: 1000, reconnectionAttempts: 10})
                mutate(tempWs)
