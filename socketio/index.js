...
async function newSocket(socket) {
    ...
    socket.emit('chat:emojis', guild?.emojis?.cache.map(e => {
        return {
            name: e.name,
            url: e.url
        }
...
function socketLogin(socket, token) {
    ...
    // Convert userId to number for consistency with API endpoints
    socket.userId = parseInt(valid.uid);
    socket.emit('auth', { success: true, userId: socket.userId });

    // Join room using userId as string (socket.io rooms are strings)
...
io.on('connection', function(socket) {
    ...
    socket.on('auth', function(token) {
        ...
        if (socket.userId) {
            // Leave old room using string version
        ...
        // Update online users count after login
    ...
    socket.on('disconnect', () => {
        // Update online users count when a user disconnects
        sendOnlineUsers(io, true);
    });
