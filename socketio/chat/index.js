...
async function joinChat(socket, channel) {
    ...
    // Update online users count after joining a channel
    const { sendOnlineUsers } = require('./functions');
    const io = require('../server');
    sendOnlineUsers(io, true);
...
async function sendMessage(socket, message, replyTo) {
    ...
    if (message.startsWith(cmdPrefix)) {
        ...
        // Check if user has permission for the command (suppress "invalid command" for permission issues)
        const [[user]] = await sql.query('SELECT perms FROM users WHERE id = ?', [socket.userId]);
        const userPerms = user?.perms || 0;

        // Moderator commands require perms >= 5
        const modCommands = ['clear', 'delete', 'lock', 'unlock', 'mute', 'unmute', 'ban', 'unban', 'tempban'];
        // Admin commands require perms >= 10
        const adminCommands = ['permission', 'balanceupdate', 'xp', 'promocode', 'removepromo'];

        if (modCommands.includes(commandName) && userPerms < 5) {
            return; // Silently ignore
        }

        if (adminCommands.includes(commandName) && userPerms < 10) {
            return; // Silently ignore
        }
